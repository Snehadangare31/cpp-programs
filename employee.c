...
void display(struct employee e)
{
    ...
    printf("\nname\t\tmob\t\tid\t\tdesignation\tts\n");
    printf("\n%s\t\t%s\t%s\t%s\t\t%f",e1.name,e1.mob,e1.id,e1.des,e1.ts);
    printf("\n%s\t\t%s\t%s\t%s\t\t%f",e2.name,e2.mob,e2.id,e2.des,e2.ts);
    printf("\n%s\t\t%s\t%s\t%s\t\t%f",e3.name,e3.mob,e3.id,e3.des,e3.ts);
    printf("\n");           
}
